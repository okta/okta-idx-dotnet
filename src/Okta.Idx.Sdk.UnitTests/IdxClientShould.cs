...
namespace Okta.Idx.Sdk.UnitTests
{
   ...
   ""error"":""some error message""
    ...
    ""app"": {
        ...
        [Fact]
        public async Task SelectOktaVerifyAsSecondFactor()
        {
            var identifyResponse = @"{
    ""version"": ""1.0.0"",
    ...
    ""app"": {
        ...
        ""value"": {
            ...
            authResponse.OktaVerifyEnrollOptions.QrCode.Should().NotBeNull();

            authResponse.AuthenticationStatus.Should().Be(AuthenticationStatus.AwaitingAuthenticatorVerification);
            authResponse.CurrentAuthenticator.Id.Should().Be("auttzfsi7LmtMLrny5d6");
            
            authResponse.OktaVerifyEnrollOptions.Should().NotBeNull();
            OktaVerifyRemediationParameter[] parameters = authResponse.OktaVerifyEnrollOptions.GetChannelOptions().ToArray();
